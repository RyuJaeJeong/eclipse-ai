package com.finance.eclipse.suggestion.preference;
...
public final class ContextPreferences {
	...
	private static void loadPreferences() {
		...
		for (final String keyString : blacklistString.split(",")) {
			...
			if (optional.isPresent()) {
				...
				AiActivator.log().warn(String.format("Failed to read blacklist key: '%s'", keyString));
			}
		}
		...
		for (final String keyString : stickylistString.split(",")) {
			...
			if (optional.isPresent()) {
				...
				AiActivator.log().warn(String.format("Failed to read sticky key: '%s'", keyString));
			}
		}
	...
	private static void savePreferences() {
		...
		try {
			...
			AiActivator.log().error("Failed to save preferences", exception);
		}
	}
